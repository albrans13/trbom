...
ask_ques = "**• ذا كنت تريد تنصب ميوزك اختار  بايروجرام❤️‍🔥\n\n• واذا تريد تنصب تلثيون فأختار تيرمڪس\n\n• يوجد استخرجات جلسات لي البــــوتات**"
...
buttons_ques = [
    ...
    [
        InlineKeyboardButton(" بايروجرام❤️‍🔥  💎", callback_data="pyrogram_bot"),
        InlineKeyboardButton("تلثيون بــــوت 💎", callback_data="telethon_bot"),
    ],
]

gen_button = [
    [
        InlineKeyboardButton(text="[{اضـعـط لـبـدا اسـتـخـراج الـكـود}] ٰدْٖ 💎 ", callback_data="generate")
    ]
]
...
async def generate_session(bot: Client, msg: Message, telethon=False, is_bot: bool = False):
    ...
    if api_id_msg.text == "/skip":
        ...
        try:
            ...
            return
        api_hash_msg = await bot.ask(user_id, "» حسنا قم بأرسال ال API_HASH", filters=filters.text)
    ...
    if not is_bot:
        await msg.reply(" انتظر سوف نرسل كود لحسابك❤️‍🔥بُِالتلجرام .")
    else:
    ...
    if not is_bot:
        ...
        try:
            ...
            try:
                two_step_msg = await bot.ask(user_id, " ارسل الاسود  الخاص بحسابك  .", filters=filters.text, timeout=300)
            except TimeoutError:
